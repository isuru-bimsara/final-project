...
const UserProfile = () => {
  ...
  // 🔥 UPDATED: Enhanced saveProfile function with proper event dispatching
  const saveProfile = async () => {
    try {
      ...
      // 🔥 CRITICAL: Update localStorage to keep data in sync
      localStorage.setItem("userName", data.user.name || "");
      localStorage.setItem("email", data.user.email || "");
      localStorage.setItem("role", data.user.role || "");

      // 🔥 CRITICAL: Dispatch event to notify Navbar about profile update
  ...
  // 🔥 UPDATED: Enhanced password change with event dispatching
  const handlePasswordChange = async (e) => {
    ...
    try {
      ...
      // 🔥 OPTIONAL: Dispatch event if password change affects user data
      window.dispatchEvent(new Event("profile:updated"));
    } catch (error) {
  ...
  // 🔥 UPDATED: Enhanced sign out handler
    ...
    return (
      <div className="min-h-screen flex flex-col bg-gray-100">
        <div className="flex-grow flex flex-col items-center justify-center p-8">
          ...
          <div className="mt-4 animate-spin rounded-full h-12 w-12 border-b-2 border-blue-600"></div>
        </div>
      </div>
    );
  }
